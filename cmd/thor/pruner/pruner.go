...
func New(db *muxdb.MuxDB, repo *chain.Repository) *Pruner {
	...
	p.goes.Go(func() {
		if err := p.pruneStorageTries(); err != nil {
			if err != context.Canceled {
				log.Warn("pruneStorageTries interrupted", "error", err)
			}
		}
	})
...
func (p *Pruner) xx() error {
	...
	for {
		...
		if n+step+128 < block.Number(bc.HeadID()) {
			...
			if err = j.Iterate(rng, func(pair kv.Pair) bool {
				i := 0
				err = j.Batch(func(p kv.Putter) error {
...
func (p *Pruner) cleanTrie(tr *muxdb.Trie, lastVer uint32, onLeaf func(key, blob, extra []byte) error) (int, error) {
	var (
		// path -> ver
	...
	it := tr.NodeIterator(nil, func(path []byte, ver uint32) bool {
		// if len(path) > 7 {
		// 	return false
		// }
		// if ver > 0 {
		// 	m[muxdb.EncodePath(path)] = ver
		// }
		return ver > lastVer
	})
	...
	for it.Next(true) {
		...
		if !it.Hash().IsZero() && len(it.Path()) <= 15 {
			ver := it.Ver()
			m[muxdb.EncodePath(it.Path())] = ver
			if it.Short() {
				sk := it.ShortKey()
				// skip last path entry which is term or belongs to child
				// if it's leaf node(has term)
				// if not, skip last path entry
				for i := 0; i < len(sk)-1; i++ {
					// if sk[i] > 15 {
					// 	break
					// }
					k := append(it.Path(), sk[:i+1]...)
					if len(k) <= 15 {
						m[muxdb.EncodePath(k)] = ver
					} else {
						break
					}
				}
				// for i := 1; i < len(sk) && i+len(it.Path()) <= 15; i++ {
				// 	m[muxdb.EncodePath(append(it.Path(), sk[:i]...))] = ver
				// }
			}
		}

		if it.Leaf() && onLeaf != nil {
			if err := onLeaf(it.LeafKey(), it.LeafBlob(), it.LeafMeta()); err != nil {
				return 0, err
			}
		}
	...
	// if err := bk.Batch(func(putter kv.Putter) error {
	// 	var err error
...
func (p *Pruner) pruneIndexTrie() error {
	...
	go func() {
		for {
			...
			log.Info("=====IndexTrie", "n", n, "target", target, "delete", deleteCount)
		}
	}()
	...
	for {
		...
		dn, err := p.cleanTrie(p.db.NewTrie(chain.IndexTrieName, sum.IndexRoot, sum.Header.Number()), n, nil)
		if err != nil {
			return err
		}
		deleteCount += dn
...
func (p *Pruner) pruneStorageTries() error {
	deleteCount := 0
	var n uint32
	var target uint32
	p.loadState("s", &n)
	go func() {
		for {
			<-time.After(15 * time.Second)
			log.Info("=====StorageTries", "n", n, "target", target, "delete", deleteCount)
		}
	}()

	for {
		bn := p.repo.BestBlock()
		if bn.Header().Number() > thor.MaxStateHistory {
			target = bn.Header().Number() - thor.MaxStateHistory
		}
		...
		it := p.db.NewTrie(state.AccountTrieName, header.StateRoot(), header.Number()).NodeIterator(nil, func(path []byte, ver uint32) bool {
			return ver > n
		})
...
func (p *Pruner) pruneAccTrie() error {
	...
	go func() {
		for {
			<-time.After(15 * time.Second)
			log.Info("=====AccountTrie", "n", n, "target", target, "delete", deleteCount)
		}
	}()
	...
	for {
		...
		dn, err := p.cleanTrie(p.db.NewTrie(state.AccountTrieName, header.StateRoot(), header.Number()), n, nil)
		if err != nil {
			return err
		}
		deleteCount += dn
