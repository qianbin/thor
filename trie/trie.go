...
func (v CompositValue) Split() ([]byte, []byte, []byte, error) {
	...
	_, _, extra, err := rlp.Split(rest)
	if err != nil {
		return nil, nil, nil, err
	}
	vers := rest[:len(rest)-len(extra)]
	return node, vers, extra, nil
}
...
func (t *Trie) TryGet(key []byte) ([]byte, error) {
	...
	value, newroot, didResolve, _, err := t.tryGet(t.root, rawKey, key, 0)
...
func (t *Trie) TryGetExtra(key []byte) ([]byte, []byte, error) {
	rawKey := key
	key = keybytesToHex(key)
	value, newroot, didResolve, extra, err := t.tryGet(t.root, rawKey, key, 0)
	if err == nil && didResolve {
		t.root = newroot
	}
...
func (t *Trie) tryGet(origNode node, rawKey, key []byte, pos int) (value []byte, newnode node, didResolve bool, extra []byte, err error) {
	switch n := (origNode).(type) {
	case nil:
		return nil, nil, false, nil, nil
	case valueNode:
		return n, n, false, nil, nil
	case *shortNode:
		if len(key)-pos < len(n.Key) || !bytes.Equal(n.Key, key[pos:pos+len(n.Key)]) {
			// key not found in trie
			return nil, n, false, nil, nil
		}
		value, newnode, didResolve, extra, err = t.tryGet(n.Val, rawKey, key, pos+len(n.Key))
		...
		return value, n, didResolve, extra, err
	case *fullNode:
		value, newnode, didResolve, extra, err = t.tryGet(n.Children[key[pos]], rawKey, key, pos+1)
		if key[pos] == 16 {
			extra = n.flags.extra
		}
		if err == nil && didResolve {
	...
	case *hashNode:
		if dr, ok := t.db.(DirectReader); ok {
			var found bool
			if value, extra, found, err = dr.DirectGet(rawKey, n.ver); err != nil {
				return nil, nil, false, nil, err
			}

			if found {
				add(int64(pos))
				return value, nil, false, extra, nil
			} else {
				setMiss()
			}
		}
		...
		if err != nil {
			return nil, n, true, nil, err
		}
		value, newnode, _, extra, err := t.tryGet(child, rawKey, key, pos)
		return value, newnode, true, extra, err
	default:
...
func (t *Trie) TryUpdate(key, value []byte) error {
	k := keybytesToHex(key)
	if len(value) != 0 {
		_, n, err := t.insert(t.root, nil, k, valueNode(value), nil)
		if err != nil {
			return err
		}
		t.root = n
...
func (t *Trie) TryUpdateExtra(key, value []byte, extra []byte) error {
	k := keybytesToHex(key)
	if len(value) != 0 {
		if len(value) < 32 {
			panic("...")
		}
		_, n, err := t.insert(t.root, nil, k, valueNode(value), extra)
		if err != nil {
			return err
		}
		t.root = n
...
func (t *Trie) insert(n node, prefix, key []byte, value node, extra []byte) (bool, node, error) {
	if len(key) == 0 {
	...
	case *shortNode:
		...
		if matchlen == len(n.Key) {
			dirty, nn, err := t.insert(n.Val, append(prefix, key[:matchlen]...), key[matchlen:], value, extra)
			if !dirty || err != nil {
				return false, n, err
			}
			flag := t.newFlag()
			// leaf
			if _, ok := nn.(valueNode); ok {
				flag.extra = extra
			}
		...
		var err error
		_, branch.Children[n.Key[matchlen]], err = t.insert(nil, append(prefix, n.Key[:matchlen+1]...), n.Key[matchlen+1:], n.Val, n.flags.extra)
		if err != nil {
			return false, nil, err
		}
		_, branch.Children[key[matchlen]], err = t.insert(nil, append(prefix, key[:matchlen+1]...), key[matchlen+1:], value, extra)
	...
	case *fullNode:
		dirty, nn, err := t.insert(n.Children[key[0]], append(prefix, key[0]), key[1:], value, extra)
		if !dirty || err != nil {
			return false, n, err
		}
		n = n.copy()
	...
	case *hashNode:
		...
		dirty, nn, err := t.insert(rn, prefix, key, value, extra)
		if !dirty || err != nil {
			return false, rn, err
		}
		return true, nn, nil
...
func (t *Trie) delete(n node, prefix, key []byte) (bool, node, error) {
	...
	case *shortNode:
		...
		case *shortNode:
			...
			// other nodes.
			flag := t.newFlag()
			if len(child.flags.extra) > 0 {
				flag.extra = child.flags.extra
			}
...
func (t *Trie) resolveHash(n *hashNode, prefix []byte, scaning bool) (node, []byte, error) {
	if ex, ok := t.db.(DatabaseReaderEx); ok {
		...
		encNode, encVers, extra, err := enc.Split()
		if err != nil {
			panic(fmt.Sprintf("split node %x: %v", n.hash, err))
		}

		var cVers []uint32
		if err := rlp.DecodeBytes(encVers, &cVers); err != nil {
			panic(fmt.Sprintf("decode node child versions %x: %v", n.hash, err))
		}

		dec := mustDecodeNode(n, encNode, cVers)
	...
	return mustDecodeNode(n, enc, nil), enc, nil
}
...
func (t *Trie) hashRoot(db DatabaseWriter, newVer uint32) (node, node, error) {
	...
	hashed, cached, err := h.hash(t.root, db, nil, true, newVer)
