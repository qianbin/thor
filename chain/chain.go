...
func (c *Chain) NewIterator(headID thor.Bytes32, fromNum uint32) *Iterator {
	return &Iterator{
		chain:   c,
		headID:  headID,
		nextNum: fromNum,
...
func (i *Iterator) Next() bool {
	...
	if bufLen := len(i.buffer); bufLen > 1 {
		// pop last one
		i.buffer = i.buffer[:bufLen-1]
		return true
	}
	...
	toNum := i.nextNum + 16 // buffer size
...
func (i *Iterator) Block() *block.Block {
	if bufLen := len(i.buffer); bufLen > 0 {
		return i.buffer[bufLen-1]
	}
	return nil
}
