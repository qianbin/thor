...
func TestIterator(t *testing.T) {
	...
	it := ch.NewIterator(ch.BestBlock().Header().ID(), 0)
	for it.Next() {
	...
	// fromNum == head
	ids = nil
	it = ch.NewIterator(ch.BestBlock().Header().ID(), ch.BestBlock().Header().Number())
	for it.Next() {
		ids = append(ids, it.Block().Header().ID())
	}
	assert.Nil(t, it.Error())
	...
	// fromNum beyond head
	it = ch.NewIterator(ch.BestBlock().Header().ID(), ch.BestBlock().Header().Number()+1)
