...
func (t *Trie) GetMeta(key []byte) ([]byte, []byte, error) {
	obj, err := t.lazyInit()
	if err != nil {
		return nil, nil, err
	}
	return obj.TryGetMeta(t.hashKey(key, false))
}
...
func (t *Trie) UpdateMeta(key, val, meta []byte) error {
	obj, err := t.lazyInit()
	if err != nil {
		return err
	}
	...
	t.journal = append(t.journal, &TrieJournalItem{
		append([]byte(nil), k...),
		append([]byte(nil), val...),
		append([]byte(nil), meta...),
	})
	return obj.TryUpdateMeta(k, val, meta)
}
...
func (t *Trie) NodeIterator(start []byte, filter func([]byte, uint32) bool) trie.NodeIterator {
	obj, err := t.lazyInit()
	if err != nil {
		return &errorIterator{err}
	}
	return obj.NodeIteratorWithFilter(start, filter)
}
...
func (i *errorIterator) LeafMeta() []byte                  { return nil }
